...
function selecionarSaborEternit(event, botao) {
    ...
    let mensagem = "Escolha o sabor do Baly:\n\n";

    sabores.forEach((sabor, index) => {
        mensagem += `${index + 1} - ${sabor}\n`;
    });
    ...
    if (isNaN(numero) || numero < 1 || numero > sabores.length) {
        alert("Escolha um sabor válido.");
        return;
    }

    const saborEscolhido = sabores[numero - 1];

    // Adiciona ao carrinho
    adicionarCesta(
        event,
        `Dose Eternit c/Baly - ${saborEscolhido}`,
        10.00,
        botao
    );
}
